...
bool CartesianPlanner::Plan(const StartState &state, DiscretizedTrajectory &result) {
  ...
  std::cout<<"start : x: "<<state.x<<" y: "<<state.y<<" theta:"<<state.theta<<" v: "<<state.v<<" phi: "<<state.phi
  <<" a : "<<state.a << std::endl;
/////////////////for plot
...
///////////////////////////
  States optimized;
  for(int i = 0;i<coarse_trajectory.data().size();i++){
    std::cout<<" i== "<<i<<std::endl;
    std::cout<<"coarse_trajectory : x:"<<coarse_trajectory.data().at(i).x<<std::endl;
    std::cout<<"coarse_trajectory : y:"<<coarse_trajectory.data().at(i).y<<std::endl;
    std::cout<<"coarse_trajectory : theta"<<coarse_trajectory.data().at(i).theta<<std::endl;
  }
