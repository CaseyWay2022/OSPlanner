...
def generate_center_line(config, start_x=0.0, start_y=0.0, start_yaw=0.0, resolution=0.1, left_bound=2.5,
    ...
    for seg in config:
        if isinstance(seg, list):
            ...
            point_count = np.abs(arc_length) / resolution
            
            print("point_count-----------------", point_count.astype('uint32'), np.abs(arc_length) , resolution)
            angles = np.linspace(start_angle, end_angle, point_count.astype('uint32'))
...
def main():
    ...
    if 'serialize' in sys.argv:
        ...
        with open(pickle_path, 'wb+') as f:
            print(type(center_line.points),type(static_obstacles), type(dynamic_obstacles),  )
